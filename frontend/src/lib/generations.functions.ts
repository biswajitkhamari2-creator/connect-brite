...
async function runOne(
  ...
  const prompt = `${promptFor(outputType, subject, chunk, options)}\n\n${
    searchVerification 
      ? `[Google Search Verification Context]\n${searchVerification}\n\nStrict Rule: Use the verified facts/data from the search context above to ensure the generated contents are fully accurate and up-to-date.\n\n`
      : ""
...
export const generateOutput = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { createOllamaGateway, OLLAMA_MODEL_NAME, UPSC_SYSTEM_PROMPT } = await import("./ai-gateway.server");
    const { generateText } = await import("ai");
    const gw = createOllamaGateway();
    ...
    // Get verified search context from Google
    let searchVerificationContext = "";
    try {
      const { searchWeb } = await import("./search.server");
      searchVerificationContext = await searchWeb(doc.title || doc.subject || "UPSC", 2);
    } catch (err) {
    ...
    try {
      ...
      for (let i = 0; i < chunks.length; i++) {
        try {
          results[i] = await runOne(gw, OLLAMA_MODEL_NAME, UPSC_SYSTEM_PROMPT, data.outputType, docSubject, chunks[i], generateText, opts, searchVerificationContext);
        } catch (err: any) {
          const msg = String(err?.message || err);
...
export const processChunk = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      const part = await runOne(gw, OLLAMA_MODEL_NAME, UPSC_SYSTEM_PROMPT, data.outputType, doc.subject ?? null, chunks[data.chunkIndex], generateText, opts, searchVerificationContext);
      return { part };
    } catch (e) {
...
export const finalizeGeneration = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const doc = await loadDocForUser(context.supabase, context.userId, data.documentId);
    const { OLLAMA_MODEL_NAME } = await import("./ai-gateway.server");
